...
export async function GET(request: Request) {
  ...
  if (oauthError) {
    return NextResponse.redirect(appRedirectUrl("/?tab=mail&mail=outlook_denied", request));
  }
  const code = params.get("code");
  const state = params.get("state");
  if (!code || !state) {
    return NextResponse.redirect(
      appRedirectUrl("/?tab=mail&mail=outlook_error&message=missing_code", request)
    );
  }
  try {
    const ctx = await prepareApiContext(request);
    await outlookService.handleCallback(code, state, ctx.userId);
    return NextResponse.redirect(appRedirectUrl("/?tab=mail&mail=outlook_connected", request));
  } catch (error) {
    ...
    return NextResponse.redirect(
      appRedirectUrl(
        `/?tab=mail&mail=outlook_error&message=${encodeURIComponent(message)}`,
        request
      )
    );
  }
